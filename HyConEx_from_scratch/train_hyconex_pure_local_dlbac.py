"""
HyConEx PUR + explication hyperréseau local (W(z)·z) sur DLBAC.

- Prédiction : classifieur dynamique local (HyConEx)
- Explications : hypernet local + pont vers oh_* + contrefactuels

Usage :
    python train_hyconex_pure_local_dlbac.py --dataset u4k-r4k-auth11k amazon1 --save
"""
...
RESULTS_DIR = ROOT / "results" / "hyconex_pure_local_dlbac"
...
def train_one(spec, *, save_dir: Path | None = None, verbose: bool = True) -> dict:
    ...
    if verbose:
        print(f"\n=== {spec.name} (HyConEx PUR + hypernet local) ===", flush=True)
    ...
    return {
        "dataset": spec.name,
        "model": "HyConExLocalModel",
        "pure_hyconex": True,
        "local_hypernet_explanation": True,
...
def main() -> None:
    p = argparse.ArgumentParser(description="HyConEx pur + hypernet local sur DLBAC")
    ...
    for name in args.dataset:
        ...
        loc = row["example_local_hypernet"]
        print(f"  Local: {loc['summary_text']}", flush=True)
